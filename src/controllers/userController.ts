...
//       const { email, password, first_name, last_name, role, phone, clinic_id, department_id } = req.body;
...
//     try {
//       const user = await User.findById(req.user?.id).populate('department_id', 'name description');
//       if (!user) {
//         res.status(404).json({
//           success: false,
...
//         .select('first_name last_name email phone role is_active department_id')
//         .populate('department_id', 'name description')
//         .skip(skip)
//         .limit(limit)
...
//       const users = await User.find(filter)
//         .select('first_name last_name email role phone is_active created_at department_id')
//         .populate('department_id', 'name description')
//         .sort({ first_name: 1, last_name: 1 });
...
//       const users = await User.find(filter)
//         .select('_id email first_name last_name role phone is_active tenant_id created_at department_id')
//         .populate('department_id', 'name description')
//         .skip(skip)
//         .limit(limit)
//         .sort({ created_at: -1 });
...
//       const user = await User.findOne(filter).populate('department_id', 'name description');
